...
export class FacturacionPage implements OnInit {
  ...
  constructor(private utilsSvc: UtilsService, private authService: AuthService,
  private AlertController: AlertController,
  public LoadingController: LoadingController,
  private _route:ActivatedRoute, private http: HttpClient, public NavCtrl: NavController,
  ) { }
  ...
  async ngOnInit() {
    ...
    await this.respuesta.forEach((element) => { console.table(element)
   ...
   /* if (this.folio.length !== 16 || this.folio.length)
    ...
    if (this.rfc.length < 12 && this.rfc.length > 13)
    {
      const alert = this.AlertController.create({
        header: 'LERDO DIGITAL',
        cssClass: 'alertDanger',
        message: 'El RFC no debe ser menor a 12 dígitos ni mayor a 13, favor de rectificar.',
        buttons: ['Aceptar']
      });
      (await alert).present();
      loading.dismiss();
...
debugger;
    ...
    if (cliente.codigo <= 0) {
      const alert = this.AlertController.create({
        header: 'LERDO DIGITAL',
        cssClass: 'alertDanger',
        message: 'El RFC ingresado no tiene un registro, debe darse de alta para poder facturar',
        buttons: ['Aceptar']
      });
      (await alert).present();
    ...
    if (regimen_cliente.codigo <= 0) {
      const alert = this.AlertController.create({
        header: 'LERDO DIGITAL',
        cssClass: 'alertDanger',
        message: 'El RFC ingresado no tiene un Regimen Fiscal Asignado, debe registrarlo para poder facturar',
        buttons: ['Aceptar']
      });
      (await alert).present();
      await loading.dismiss();
    ...
    if (this.res.codigo === 1) {
      const alert = this.AlertController.create({
        header: 'LERDO DIGITAL',
        cssClass: 'alertDanger',
        message: 'Folio Facturado correctamente, el PDF de la factura se envió al correo previamente registrado.',
        buttons: ['Aceptar']
      });
      (await alert).present();
      await loading.dismiss();
  ...
  async guardarcliente() {
    ...
    if (this.colonia === '' || this.colonia === ' ' || this.colonia === undefined) {
      const alert = this.AlertController.create({
        header: 'LERDO DIGITAL',
        cssClass: 'alertDanger',
    ...
    if (this.direccion === '' || this.direccion === ' ' || this.direccion === undefined) {
      const alert = this.AlertController.create({
        header: 'LERDO DIGITAL',
        cssClass: 'alertDanger',
    ...
    this.respuesta = await this.authService.getGuardaCliente(this.rfc,this.nombre,this.tipo_persona,this.direccion,this.colonia,this.CP,this.email,this.celular,this.estado,this.municipio,this.regimen);
  ...
  async enviaInformacion() {  
    ...
    if (this.resp_confirmacion.codigo > 0) {
      ...
      const alert = this.AlertController.create({
        header: 'LERDO DIGITAL',
        cssClass: 'alertDanger',
        message: 'Hubo un error al enviar la información, inténtelo nuevamente.',
        buttons: ['Aceptar']
      });
      (await alert).present();
  ...
  async guardarcliente_regimen()
  {
    ...
    if (this.regimen === '' || this.regimen === '0' || this.regimen === undefined) {
      const alert = this.AlertController.create({
        header: 'LERDO DIGITAL',
        cssClass: 'alertDanger',
        message: 'Debe seleccionar un Regimen Físcal según su Constancia de situación Físcal,Favor de rectificar',
        buttons: ['Aceptar']
      });
      (await alert).present();
      //loading.dismiss(); 
